...
import com.curatix.api.annotation.RateLimited;
import com.curatix.api.annotation.RequireConfiguration;
import com.curatix.api.annotation.RequirePermission;
...
public class EnhancedOperationCustomizer implements OperationCustomizer {
    ...
    private void addRateLimitHeaders(Operation operation, RateLimited rateLimited) {
        ...
        okResponse.addHeaderObject("x-ratelimit-limit", new Header()
                .schema(new IntegerSchema()).description("The number of allowed requests in the current period"));
        okResponse.addHeaderObject("x-ratelimit-remaining", new Header()
                .schema(new IntegerSchema()).description("The number of remaining requests in the current period"));
        okResponse.addHeaderObject("x-ratelimit-reset", new Header()
                .schema(new IntegerSchema()).description("The timestamp of the start of the next period"));
    }
